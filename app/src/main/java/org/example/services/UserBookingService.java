...
public class UserBookingService {
    ...
    public List<Train> getTrains(String source , String destination){
        try {
            ...
            return trainServiceObject.searchTrains(source, destination);
        }
        catch (IOException exception){
    ...
    // To book the seat in the TRAIN
    public Boolean bookTrainSeat(Train train, int row, int seat){
        try{
            TrainService trainServiceObject = new TrainService();
            List<List<Integer>> seats = train.getSeats();
            if (row >= 0 && row < seats.size() && seat >= 0 && seat < seats.get(row).size()){
                if (seats.get(row).get(seat) == 0){ // If seat of that row is equal to 0-(ZERO)
                    seats.get(row).set(seat , 1); // then set that seat of row is equal to 1-(ONE)
                    train.setSeats(seats); // To set new SEATS after booking
                    trainServiceObject.addTrain(train); // To save this train properly in file - (Local DB)
